...
# %%
@torch.inference_mode()
def prompt_cora_batch(dataset, llm_name, temperature, logits, pl_mask):
    '''
...
def answer_parser_cora_per_node(dataset, llm_name, idx):
...
def answer_parser_cora(dataset, llm_name, pl_mask):
    pl_nodes_list = torch.nonzero(pl_mask).squeeze().tolist()  
    pseudo_labels = []
    explanations = []
    for i in pl_nodes_list:
        label, explanation = answer_parser_cora_per_node(dataset, llm_name, i)
...
# %%
def prompt_cora(dataset, llm_name, temperature, pl_mask, logits, gpu_nums):
    # data, num_classes = load_data('cora', use_dgl=False, use_text=False, seed=0)
    folder_path = f'/storage/qiaoyr/TAPE/prompts/LLMs/{llm_name}/{dataset}'
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    with torch.no_grad():
        prompt_cora_batch(dataset, llm_name, temperature, logits, pl_mask)
    pl, explanations = answer_parser_cora(dataset, llm_name, pl_mask)
    print(pl)
