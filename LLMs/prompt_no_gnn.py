...
def prompt_per_node(dataset, node_idx):
    category = category_list_dict[dataset]
    file_path = f'/storage/qiaoyr/TAPE/prompts/{dataset}/{dataset}_{node_idx}.json'
    with open(file_path, 'r') as f:
        prompt = json.load(f)
    prompt['conversations']['value'] = prompt['conversations']['value'].replace('/ GNN-predicted','')
    prompt['conversations']['value'] = prompt['conversations']['value'].replace('/ "GNN-predicted node label"','')
    return prompt
# %%
@torch.inference_mode()
def prompt_batch(dataset, llm_name, temperature, pl_mask):
    '''
    ...
    for idx in tqdm(nodes):
        prompt = prompt_per_node(dataset, idx)
...
# %%
def prompt_LLM(cfg, pl_mask):
    dataset = cfg.dataset
    llm_name = cfg.llm.name
    temperature = cfg.llm.temperature
    # data, num_classes = load_data('cora', use_dgl=False, use_text=False, seed=0)
    folder_path = f'/storage/qiaoyr/TAPE/prompts/LLMs/{llm_name}_nognn/{dataset}'
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    with torch.no_grad():
        prompt_batch(dataset, llm_name, temperature, pl_mask)
