...
def find_and_set_free_gpus(num_gpus):
    try:
        # 获取所有可用的GPU
        gpus = GPUtil.getGPUs()
        ...
        for gpu in gpus:
            # 检查GPU是否空闲
            if gpu.memoryFree > 0.1 and gpu.memoryUtil < 0.1:
                free_gpus.append(gpu)
                if len(free_gpus) == num_gpus:
                    break  # 找到指定数量的空闲GPU则停止搜索
        
        if len(free_gpus) < num_gpus:
            print(f"只找到了 {len(free_gpus)} 个空闲GPU，少于所需的数量。")
            return None
        
        # 获取找到的GPU的ID列表
        free_gpu_ids = [gpu.id for gpu in free_gpus]
        
        # 将找到的GPU的ID转换为字符串格式，用逗号分隔
        gpu_ids_str = ','.join(str(gpu_id) for gpu_id in free_gpu_ids)
        
        # 设置CUDA_VISIBLE_DEVICES环境变量
        os.environ["CUDA_VISIBLE_DEVICES"] = gpu_ids_str
        
        print(f"已将以下GPU指定为使用的GPU: {free_gpu_ids}")
        
        return free_gpu_ids  # 返回找到的空闲GPU的ID列表
...
import os
import sys
sys.path.append('/storage/qiaoyr/GraphGPT')
import graphgpt
...
def run_eval(args, num_gpus):
    ...
    print("prompt file length: ", len(prompt_file))
    print("prompt file example: ", prompt_file[0])
...
if __name__ == "__main__":
    ...
    free_gpu_ids = find_and_set_free_gpus(num_requested_gpus)
