...
from core.paths import dataset_root


def get_raw_text_arxiv(use_text=False, seed=0):

    dataset = PygNodePropPredDataset(
        name='ogbn-arxiv', transform=T.ToSparseTensor(), root=str(dataset_root() / 'ogbn_arxiv'))
    data = dataset[0]
    ...
    nodeidx2paperid = pd.read_csv(
        str(dataset_root() / 'ogbn_arxiv' / 'mapping' / 'nodeidx2paperid.csv.gz'), compression='gzip')

    raw_text = pd.read_csv(str(dataset_root() / 'ogbn_arxiv_orig' / 'titleabs.tsv'),
