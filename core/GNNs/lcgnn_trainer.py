...
from core.paths import TAPE_ROOT, DATASET_ROOT, PROMPTS_ROOT, PRT_LM_ROOT, llm_response_dir, node_prompt_path, prt_lm_emb_path
...
class LCGNNTrainer():
    ...
    def __init__(self, cfg, feature_type, data, num_classes):
        ...
        if self.feature_type == 'llm': # stored in folder embedding
            ...
            LM_emb_path = f"{TAPE_ROOT}/prt_lm_1/{self.dataset_name}/{self.lm_model_name}-seed{self.seed}.emb"
            ...
            print("Loading pretrained LM features (title and abstract) ...")
            LM_emb_path = prt_lm_emb_path(self.dataset_name, self.lm_model_name, self.seed)
            print(f"LM_emb_path: {LM_emb_path}")
            features = torch.from_numpy(np.array(
                np.memmap(LM_emb_path, mode='r',
                          dtype=np.float16,
