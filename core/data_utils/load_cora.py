...
def get_cora_casestudy(SEED=0):
    ...
    dataset = Planetoid(str(dataset_root()), data_name,
...
def parse_cora():
    path = str(dataset_root() / 'cora_orig' / 'cora')
...
def get_raw_text_cora(use_text=False, seed=0):
    ...
    with open(dataset_root() / 'cora_orig' / 'mccallum' / 'cora' / 'papers') as f:
        lines = f.readlines()
    pid_filename = {}
    for line in lines:
        pid = line.split('\t')[0]
    ...
    path = str(dataset_root() / 'cora_orig' / 'mccallum' / 'cora' / 'extractions') + '/'
    text = []
