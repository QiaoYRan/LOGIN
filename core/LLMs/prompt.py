import os
import torch
from core.paths import llm_output_dir, llm_response_path, prompt_path
import json
...
def prompt_per_node(dataset, node_idx, y_pred):
    ...
    file_path = str(prompt_path(dataset, node_idx))
...
def prompt_batch(dataset, llm_name, temperature, logits, pl_mask):
    ...
    for idx in tqdm(nodes):
        ...
        folder_path = llm_output_dir(llm_name, dataset)
        os.makedirs(folder_path, exist_ok=True)
        res_file = str(llm_response_path(llm_name, dataset, idx))
...
def answer_parser_per_node(dataset, llm_name, idx):
    ...
    file_path = str(llm_response_path(llm_name, dataset, idx))
    with open(file_path, 'r') as f:
        answer = json.load(f)
    # print(answer)
    # print(answer['res'])
