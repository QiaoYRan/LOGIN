...
def get_pubmed_casestudy(corrected=False, SEED=0):
    ...
    dataset = Planetoid('/storage/qiaoyr/dataset/', data_name, transform=T.NormalizeFeatures())
    data = dataset[0]
...
def parse_pubmed():
    path = '/storage/qiaoyr/TAPE/dataset/PubMed_orig/data/'
...
def get_raw_text_pubmed(use_text=False, seed=0):
    ...
    f = open('/storage/qiaoyr/TAPE/dataset/PubMed_orig/pubmed.json')
    pubmed = json.load(f)
