...
from core.paths import data_root, prt_lm_path
...
class LCGNNTrainer():
    ...
    def __init__(self, cfg, feature_type, data, num_classes):
        ...
        if self.feature_type == 'llm': # stored in folder embedding
            ...
            LLM_emb_path = str(data_root() / "llm_emb" / self.dataset_name / f"{self.llm_model_name}.npy")
            ...
            LM_emb_path = str(prt_lm_path(self.dataset_name, self.lm_model_name, self.seed, variant="re"))
            ...
            print("Loading pretrained LM features (title and abstract) ...")
            LM_emb_path = str(prt_lm_path(self.dataset_name, self.lm_model_name, self.seed, variant="TA"))
            print(f"LM_emb_path: {LM_emb_path}")
            features = torch.from_numpy(np.array(
                np.memmap(LM_emb_path, mode='r',
                          dtype=np.float16,
            ...
            LM_emb_path = str(data_root() / "prt_lm" / f"{self.dataset_name}2" / f"{self.lm_model_name}-seed{self.seed}.emb")
            print(f"LM_emb_path: {LM_emb_path}")
            features = torch.from_numpy(np.array(
                np.memmap(LM_emb_path, mode='r',
                          dtype=np.float16,
    ...
    def set_pl_mask(self, pl_mask):
        ...
        # Keep supervision strictly on training nodes to avoid label leakage.
        self.gold_mask = self.data.train_mask & (~self.pl_mask)
    ...
    def update_pseudo_labels_and_features(self, pseudo_labels, emb): # pseudo_labels: list
        ...
        valid_pl_nodes = [pl_nodes_list[i] for i in valid_indices]
        emb = emb[valid_indices].to(self.device) if len(valid_indices) > 0 else emb[:0].to(self.device)
        print('emb_shape:', emb.shape)
        print('orig_features_shape:', self.features.shape)
        print('pl_nodes_list_len:', len(valid_pl_nodes))

        if len(valid_pl_nodes) > 0:
            self.features[valid_pl_nodes] = emb
    ...
    def augment_adjacency_matrix_sim(self):
        # Paper-consistent oracle inconsistency: compare pseudo labels with ground truth.
        inconsistency = (self.data.y != self.pseudo_labels) & self.pl_mask
        inconsistent_nodes_list = torch.nonzero(inconsistency).squeeze().tolist()
        if isinstance(inconsistent_nodes_list, int):
            inconsistent_nodes_list = [inconsistent_nodes_list]
        if len(inconsistent_nodes_list) == 0:
            print('inconsistent nodes list: []')
            return
        
        print('inconsistent nodes list:', inconsistent_nodes_list)
        edge_index = self.data.edge_index
        if self._edge_index_backup is None:
            self._edge_index_backup = edge_index.clone()
    ...
    def restore_original_graph(self):
        if self._edge_index_backup is not None:
            self.data.edge_index = self._edge_index_backup
            self._edge_index_backup = None
