...
def prompt_per_node(dataset, node_idx, y_pred):
    ...
    file_path = node_prompt_path(dataset, node_idx)
...
def prompt_batch(dataset, llm_name, temperature, logits, pl_mask):
    ...
    for idx in tqdm(nodes):
        ...
        folder_path = llm_response_dir(llm_name, dataset)
...
def answer_parser_per_node(dataset, llm_name, idx):
    ...
    file_path = os.path.join(llm_response_dir(llm_name, dataset), f"{dataset}_{idx}.json")
    with open(file_path, 'r') as f:
        answer = json.load(f)
    # print(answer)
    # print(answer['res'])
