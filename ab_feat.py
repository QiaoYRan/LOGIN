...
def run(cfg):
    ...
    for seed in seeds:
        ...
        # update features
        trainer_lc = TRAINER(cfg, 'TA', data, num_classes)
    ...
    if len(all_acc_lc) > 1:
        ...
        print(f"[w\o feat:{cfg.gnn.model.name} + {cfg.gnn.train.feature_type} + (GNN+LLM)] ValACC: {df['val_acc'].mean():.4f} ± {df['val_acc'].std():.4f}, TestAcc: {df['test_acc'].mean():.4f} ± {df['test_acc'].std():.4f}")
