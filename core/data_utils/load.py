...
def load_data(dataset, use_dgl=False, use_text=False, use_gpt=False, seed=0):
    if dataset == 'cora':
        ...
        num_classes = 40
    elif dataset == 'wisconsin' or dataset == 'cornell' or dataset == 'texas' or dataset == 'washington':
        from core.data_utils.load_webkb_1 import get_raw_text_webkb as get_raw_text
        num_classes = 5
    else:
    ...
    # for training GNN
    if not use_text:
        if dataset == 'wisconsin' or dataset == 'cornell' or dataset == 'texas':
            data, _ = get_raw_text(dataset, use_text=False, seed=seed)
        else:
    ...
    #             text.append(content)
        ...
        if dataset == 'wisconsin' or dataset == 'cornell' or dataset == 'texas' or dataset == 'washington':
            data, text = get_raw_text(dataset, use_text=True, seed=seed)
        else:
