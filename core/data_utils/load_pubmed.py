...
def get_pubmed_casestudy(corrected=False, SEED=0):
    ...
    dataset = Planetoid(f'{DATASET_ROOT}/', data_name, transform=T.NormalizeFeatures())
    data = dataset[0]
...
def parse_pubmed():
    path = f'{TAPE_ROOT}/dataset/PubMed_orig/data/'
...
def get_raw_text_pubmed(use_text=False, seed=0):
    ...
    f = open(os.path.join(DATASET_ROOT, 'PubMed_orig', 'pubmed.json'))
    pubmed = json.load(f)
